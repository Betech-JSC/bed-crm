<?php

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| Routes are organized into modular files under routes/modules/.
| Each module handles a specific domain of the CRM application.
|
*/

require __DIR__ . '/modules/auth.php';
require __DIR__ . '/modules/dashboard.php';
require __DIR__ . '/modules/leads.php';
require __DIR__ . '/modules/deals.php';
require __DIR__ . '/modules/contacts.php';
require __DIR__ . '/modules/sales.php';
require __DIR__ . '/modules/marketing.php';
require __DIR__ . '/modules/automation.php';
require __DIR__ . '/modules/settings.php';
require __DIR__ . '/modules/customers.php';
require __DIR__ . '/modules/support-tickets.php';
require __DIR__ . '/modules/projects.php';
require __DIR__ . '/modules/hr.php';
require __DIR__ . '/modules/finance.php';
require __DIR__ . '/modules/intelligence.php';
require __DIR__ . '/modules/notifications.php';
require __DIR__ . '/modules/rbac.php';
require __DIR__ . '/modules/strategy.php';
require __DIR__ . '/modules/organization.php';
require __DIR__ . '/modules/casestudies.php';
require __DIR__ . '/modules/wiki.php';
require __DIR__ . '/modules/ai-chat.php';
require __DIR__ . '/modules/ai-trends.php';
require __DIR__ . '/modules/content-studio.php';
require __DIR__ . '/modules/system-logs.php';
require __DIR__ . '/modules/system-history.php';
require __DIR__ . '/modules/business.php';
require __DIR__ . '/modules/documents.php';
require __DIR__ . '/modules/dropship.php';
require __DIR__ . '/modules/dev-portal.php';
require __DIR__ . '/modules/video-ads.php';
require __DIR__ . '/modules/meetings.php';
require __DIR__ . '/modules/ceo-roadmap.php';
require __DIR__ . '/modules/culture.php';
require __DIR__ . '/modules/prompt-learning.php';
require __DIR__ . '/modules/outbound-sales.php';
require __DIR__ . '/modules/personal-tasks.php';
require __DIR__ . '/modules/web-forms.php';
require __DIR__ . '/modules/utm-tracking.php';
require __DIR__ . '/modules/landing-pages.php';
require __DIR__ . '/modules/seo-content.php';
require __DIR__ . '/modules/social-proof.php';
require __DIR__ . '/modules/growth-tools.php';
require __DIR__ . '/modules/advanced-modules.php';
require __DIR__ . '/modules/ai-data-hub.php';
require __DIR__ . '/modules/brand-foundation.php';
require __DIR__ . '/modules/showcase.php';
