<template>
  <div>
    ...
    <Card>
      <template #title>Edit Proposal</template>
      <template #content>
        <form @submit.prevent="update" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="flex flex-col md:col-span-2">
              <label class="mb-2 text-sm font-medium">Update File (Optional)</label>
              <FileUpload
                mode="basic"
                accept=".pdf,.doc,.docx"
              ...
              <small class="text-gray-500 mt-1">Leave empty to keep current file: {{ proposal.file_name }}</small>
          ...
          <div class="flex items-center justify-end gap-2 pt-4 border-t">
            ...
            <Button label="Update Proposal" icon="pi pi-check" :loading="form.processing" type="submit" />
          </div>
        </form>
      </template>
    </Card>
...
export default {
  ...
  data() {
    return {
      ...
      form: this.$inertia.form({
        deal_id: this.proposal.deal_id,
        title: this.proposal.title,
        description: this.proposal.description || '',
        amount: this.proposal.amount,
        valid_until: this.proposal.valid_until,
        file: null,
      }),
      breadcrumbItems: [
  ...
  methods: {
    ...
    update() {
      ...
      this.$inertia.post(`/proposals/${this.proposal.id}`, formData, {
        forceFormData: true,
      })
    },
  },
