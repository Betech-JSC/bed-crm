<template>
  <div>
    <Head title="Users" />
    <h1 class="mb-8 text-3xl font-bold">{{ t('common.users') }}</h1>
    <div class="flex items-center justify-between mb-6">
      <search-filter v-model="form.search" class="mr-4 w-full max-w-md" @reset="reset">
        <label class="block text-gray-700">{{ t('common.role') }}:</label>
        <select v-model="form.role" class="form-select mt-1 w-full">
          <option :value="null" />
          <option value="user">User</option>
          <option value="owner">Owner</option>
        </select>
        <label class="block mt-4 text-gray-700">{{ t('common.trashed') }}:</label>
        <select v-model="form.trashed" class="form-select mt-1 w-full">
          <option :value="null" />
          <option value="with">{{ t('common.with_trashed') }}</option>
          <option value="only">{{ t('common.only_trashed') }}</option>
        </select>
      </search-filter>
    ...
    <div class="bg-white rounded-md shadow overflow-x-auto">
      <table class="w-full whitespace-nowrap">
        <tr class="text-left font-bold">
          ...
          <th class="pb-4 pt-6 px-6">{{ t('common.email') }}</th>
