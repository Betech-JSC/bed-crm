  ...
  <div>
    ...
    <div class="editor-layout">
      <div class="editor-header">
        ...
        <div class="header-actions">
          <Link :href="`/wiki/${article.id}`">
            <Button label="Xem bài viết" icon="pi pi-eye" severity="secondary" text size="small" />
          </Link>
          <Link href="/wiki">
            <Button label="Wiki" icon="pi pi-arrow-left" severity="secondary" text size="small" />
          </Link>
        </div>
      </div>
      ...
      <div v-if="article.deleted_at" class="trashed-banner">
        ...
        <span>Bài viết này đã bị xóa. Khôi phục để tiếp tục chỉnh sửa.</span>
      </div>

      <form @submit.prevent="submit" class="editor-form">
        <div class="editor-main">
          <div class="form-group">
            <input v-model="form.title" type="text" class="title-input" placeholder="Tiêu đề bài viết..." />
            <span v-if="form.errors.title" class="error">{{ form.errors.title }}</span>
          </div>

          <div class="form-group">
            <input v-model="form.excerpt" type="text" class="excerpt-input" placeholder="Mô tả ngắn..." />
          </div>
          ...
          <div class="form-group">
            ...
            <div v-if="!previewMode">
              <textarea ref="contentEditor" v-model="form.content" class="content-editor" rows="20"
        ...
        <div class="editor-sidebar">
          <div class="sidebar-card">
            ...
            <div class="form-group">
              ...
              <div class="status-selector">
                <button type="button" class="status-btn" :class="{ active: form.status === 'draft' }" @click="form.status = 'draft'">
                  <i class="pi pi-file-edit" /> Nháp
                </button>
                <button type="button" class="status-btn published" :class="{ active: form.status === 'published' }" @click="form.status = 'published'">
            ...
            <div class="form-group">
              ...
              <input v-model="form.change_summary" type="text" class="form-control" placeholder="VD: Cập nhật quy trình..." />
            </div>
          </div>
          ...
          <div class="sidebar-actions">
            <Button
              type="submit"
              :label="form.status === 'published' ? 'Cập nhật & Xuất bản' : 'Lưu nháp'"
...
export default {
  ...
  methods: {
    submit() {
      this.form.put(`/wiki/${this.article.id}`)
    },
    insertTag(tag) {
      ...
      this.form.content = this.form.content.substring(0, start) + `<${tag}>${selected}</${tag}>` + this.form.content.substring(end)
    },
    insertHeading() {
      const el = this.$refs.contentEditor
      const start = el.selectionStart
...
.page-subtitle { font-size: 0.78rem; color: #94a3b8; margin: 0.1rem 0 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 400px; }
...
.form-control {
  ...
  border-radius: 8px; font-size: 0.82rem; outline: none; font-family: inherit; transition: all 0.2s;
}
