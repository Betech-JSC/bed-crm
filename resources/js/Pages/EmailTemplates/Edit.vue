<template>
  <div>
    <Head title="Chỉnh sửa Email Template" />
    ...
    <div class="page-header">
      <div class="header-left">
        ...
        <div>
          <h1 class="page-title">Chỉnh sửa Template</h1>
      ...
      <form @submit.prevent="submit">
        <div class="form-grid">
          <div class="form-group full">
            ...
            <input v-model="form.name" class="form-input" :class="{ 'has-error': form.errors.name }" />
          ...
          <div class="form-group">
            ...
            <input v-model="form.subject" class="form-input" :class="{ 'has-error': form.errors.subject }" />
          ...
          <div class="form-group full">
            ...
            <textarea v-model="form.body_text" class="form-textarea" rows="6" />
          </div>
...
export default {
  ...
  layout: Layout,
  props: { template: Object, types: Array, variables: Array },
  data() {
    return {
      form: useForm({
        ...
        type: this.template?.type || 'campaign',
        is_active: this.template?.is_active ?? true,
      }),
    }
  },
  methods: {
    typeLabel(t) { return { campaign: 'Campaign', automation: 'Automation', transactional: 'Transactional' }[t] || t },
    submit() { this.form.put(`/email-templates/${this.template.id}`) },
    confirmDelete() { if (confirm('Bạn có chắc muốn xóa template này?')) router.delete(`/email-templates/${this.template.id}`) },
  },
}
</script>
...
.btn-danger { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.45rem 0.85rem; border-radius: 10px; border: 1.5px solid #fecaca; background: #fef2f2; color: #dc2626; font-size: 0.78rem; font-weight: 600; cursor: pointer; transition: all 0.2s; }
