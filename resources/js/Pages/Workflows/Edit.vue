        ...
        <form @submit.prevent="update" class="space-y-6">
          ...
          <div v-for="(action, index) in form.actions" :key="index" class="p-4 border border-gray-200 rounded-lg mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div v-if="action.type === 'update_field'" class="flex flex-col">
                ...
                <InputText v-model="action.field" />
              </div>
              <div v-if="action.type === 'update_field'" class="flex flex-col">
          ...
          <div class="flex items-center justify-between pt-4 border-t">
            ...
            <div class="flex gap-2">
              ...
              <Button label="Update Workflow" icon="pi pi-check" :loading="form.processing" type="submit" />
            </div>
          </div>
        </form>
      </template>
...
export default {
  ...
  data() {
    return {
      form: this.$inertia.form({
        ...
        trigger: this.workflow.trigger || { event: null, conditions: null },
        actions: this.workflow.actions || [],
        is_active: this.workflow.is_active !== undefined ? this.workflow.is_active : true,
      }),
      statusOptions: [
  ...
  methods: {
    onTriggerChange() {
      ...
      if (!this.form.trigger.conditions) {
    ...
    update() {
      this.form.put(`/workflows/${this.workflow.id}`)
    },
    destroy() {
      if (confirm('Are you sure you want to delete this workflow?')) {
        this.$inertia.delete(`/workflows/${this.workflow.id}`)
      }
    },
  },
}
