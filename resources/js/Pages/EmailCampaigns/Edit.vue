<template>
  <div>
    <Head title="Chỉnh sửa Campaign" />
    <div class="page-header">
      <div class="header-left">
        <Link :href="`/email-campaigns/${campaign.id}`" class="back-btn"><i class="pi pi-arrow-left" /></Link>
        <div class="header-icon"><i class="pi pi-megaphone" /></div>
        <div>
          <h1 class="page-title">Chỉnh sửa Campaign</h1>
      ...
      <form @submit.prevent="submit">
        <div class="form-grid">
          ...
          <div class="form-group">
            ...
            <input v-model="form.subject" class="form-input" :class="{ err: form.errors.subject }" />
          ...
          <div class="form-group">
            ...
            <select v-model="form.email_list_id" class="form-select" :class="{ err: form.errors.email_list_id }">
              ...
              <option v-for="l in lists" :key="l.id" :value="l.id">{{ l.name }} ({{ l.contacts_count }})</option>
            </select>
          ...
          <div class="form-group full">
            ...
            <textarea v-model="form.body_html" class="form-textarea" rows="12" />
          </div>

          <div class="form-group full">
            <label class="form-label">Nội dung Text</label>
            <textarea v-model="form.body_text" class="form-textarea" rows="5" />
          </div>
        </div>

        <div class="form-actions">
          <Link :href="`/email-campaigns/${campaign.id}`"><button type="button" class="btn-cancel">Hủy</button></Link>
          <button type="submit" class="btn-submit" :disabled="form.processing">
            <i v-if="form.processing" class="pi pi-spin pi-spinner" />
            <i v-else class="pi pi-check" /> Lưu thay đổi
          </button>
        </div>
      </form>
    </div>
...
export default {
  ...
  layout: Layout,
  props: { campaign: Object, templates: Array, lists: Array },
  data() {
    return {
      form: useForm({
        ...
        body_html: this.campaign.body_html || '',
        body_text: this.campaign.body_text || '',
        email_template_id: this.campaign.email_template_id || null,
        email_list_id: this.campaign.email_list_id || null,
        scheduled_at: this.campaign.scheduled_at || null,
      }),
    }
  },
  methods: {
    submit() { this.form.put(`/email-campaigns/${this.campaign.id}`) },
    confirmDelete() { if (confirm('Bạn có chắc muốn xóa campaign này?')) router.delete(`/email-campaigns/${this.campaign.id}`) },
  },
}
</script>
...
.form-card{background:#fff;border-radius:16px;border:1.5px solid #e2e8f0;padding:1.5rem}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:1rem}.form-group{display:flex;flex-direction:column;gap:.3rem}.form-group.full{grid-column:1/-1}.form-label{font-size:.78rem;font-weight:600;color:#374151}.req{color:#ef4444}.form-input,.form-select,.form-textarea{padding:.55rem .75rem;border:1.5px solid #e2e8f0;border-radius:10px;font-size:.82rem;color:#334155;transition:all .2s;background:#fafbfc;outline:none;font-family:inherit;width:100%;box-sizing:border-box}.form-input:focus,.form-select:focus,.form-textarea:focus{border-color:#3b82f6;background:#fff;box-shadow:0 0 0 3px rgba(59,130,246,.1)}.err{border-color:#ef4444!important}.form-textarea{resize:vertical;min-height:60px}.form-error{font-size:.68rem;color:#ef4444;font-weight:500}
