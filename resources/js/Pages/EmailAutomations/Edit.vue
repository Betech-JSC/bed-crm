<template>
  <div>
    <Head title="Chỉnh sửa Automation" />
    <div class="page-header">
      <div class="header-left">
        <Link :href="`/email-automations/${automation.id}`" class="back-btn"><i class="pi pi-arrow-left" /></Link>
        <div class="header-icon"><i class="pi pi-bolt" /></div>
        <div><h1 class="page-title">Chỉnh sửa Automation</h1><p class="page-subtitle">{{ automation.name }}</p></div>
      </div>
          ...
          <div class="form-group full">
            ...
            <div class="trigger-grid">
              <div v-for="t in triggerTypes" :key="t" class="trigger-option" :class="{ active: form.trigger_type === t }" @click="form.trigger_type = t">
                <i :class="triggerIcon(t)" /><span>{{ triggerLabel(t) }}</span>
              </div>
            </div>
          </div>
        </div>
        <div class="form-actions">
          <Link :href="`/email-automations/${automation.id}`"><button type="button" class="btn-cancel">Hủy</button></Link>
...
export default {
  ...
  props: { automation: Object, triggerTypes: Array },
  data() {
    return { form: useForm({ name: this.automation.name, description: this.automation.description || '', trigger_type: this.automation.trigger_type, trigger_config: this.automation.trigger_config || {} }) }
  },
  methods: {
    submit() { this.form.put(`/email-automations/${this.automation.id}`) },
    confirmDelete() { if (confirm('Bạn có chắc muốn xóa?')) router.delete(`/email-automations/${this.automation.id}`) },
    triggerLabel(t) { return { lead_created: 'Lead mới', contact_created: 'Contact mới', deal_won: 'Deal thắng', tag_added: 'Thêm tag' }[t] || t },
    triggerIcon(t) { return { lead_created: 'pi pi-user-plus', contact_created: 'pi pi-users', deal_won: 'pi pi-star', tag_added: 'pi pi-tag' }[t] || 'pi pi-bolt' },
  },
}
</script>
<style scoped>
